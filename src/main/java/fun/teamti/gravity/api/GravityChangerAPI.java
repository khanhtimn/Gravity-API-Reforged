...
public abstract class GravityChangerAPI {
    public static final ComponentKey<GravityComponent> GRAVITY_COMPONENT =
            ComponentRegistry.getOrCreate(new ResourceLocation("gravity_api", "gravity_direction"), GravityComponent.class);

    public static final ComponentKey<GravityDimensionStrengthInterface> GRAVITY_DIMENSION_STRENGTH_COMPONENT =
            ComponentRegistry.getOrCreate(new ResourceLocation("gravity_api", "gravity_dimension_strength"), GravityDimensionStrengthInterface.class);
    ...
    * */
    ...
    @Environment(EnvType.CLIENT)
    public static void setIsInvertedClient(ClientPlayerEntity entity, boolean isInverted, RotationParameters rotationParameters, ResourceLocation verifier, FriendlyByteBuf verifierInfo) {
        if(onCorrectSide(entity, false)) {
            if (EntityTags.canChangeGravity(entity)) {
                maybeGetSafe(GRAVITY_COMPONENT, entity).ifPresent(gc -> {
                    gc.invertGravity(isInverted, rotationParameters, false);
